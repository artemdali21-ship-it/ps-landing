        ...
        <div className="hidden md:flex items-center gap-8">
          {[
            ...
            <a
              key={id}
              href={\`#\${id}\`}
              className="font-sans font-medium text-xs uppercase tracking-widest text-taupe hover:text-crimson transition-colors duration-200"
            >
              {label}
            </a>
          ))}
...
writeFileSync(join(root, "components/Navbar.tsx"), navbar, "utf8");
console.log("Navbar.tsx written successfully");
console.log("Content starts with:", navbar.substring(0, 50));
